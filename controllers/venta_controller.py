...
def create():
    if request.method == 'POST':
        ...
        venta = Venta(fecha=fecha,cantidad=cantidad,cliente_id=cliente_id,producto_id=producto_id)
        venta.save()
...
def edit(id):
    ...
    if request.method == 'POST':
        ...
        cliente_id = request.form['cliente_id']
        producto_id = request.form['producto_id']
        venta.update(fecha=fecha,cantidad=cantidad,cliente_id=cliente_id,producto_id=producto_id)
        return redirect(url_for('venta.index'))
    clientes = Cliente.query.all()
    productos = Producto.query.all()
    return venta_view.edit(venta,clientes,productos)

@venta_bp.route("/delete/<int:id>",methods=['GET','POST'])
def delete(id):
    venta = Venta.get_by_id(id)
    venta.delete()
    return redirect(url_for('venta.index'))